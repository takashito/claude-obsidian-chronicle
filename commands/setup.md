...
4. **Language** (`language`) — Header: `Language`. The language notes are written in (summary, title, section headings). **Detect the language the user has been conversing in *this session* and present it as the recommended option** (list it first, labelled "recommended") — e.g. if this conversation is in Japanese, lead with `Japanese`; if in English, lead with `English`. If a config value already exists at the chosen file, that prior choice is the recommended default instead (respect it over detection). Always also offer `English` as an alternative when it isn't already the recommended one (it is the built-in fallback). (Other = type a language name verbatim, e.g. `Français`.) Passed verbatim into the summarization prompt.
