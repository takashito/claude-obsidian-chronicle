...
obsidian-chronicle は **Claude Code プラグイン**です。Claude Code 上で次のスラッシュコマンドを実行してインストールします:
...
いちばん簡単なのは、Claude Code 上で **`/obsidian-chronicle:setup`** を実行する方法です。Vault を自動検出してこのファイルを生成してくれます。手動で編集する場合は以下を参照してください:
