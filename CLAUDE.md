...
- **Detached worker subshell with `trap '' HUP`:** the whole job runs in `( ... ) & disown` so `/clear`/quit return instantly, and survives SIGHUP when the parent dies mid-summary.
